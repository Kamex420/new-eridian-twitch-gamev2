...
SKILL_LABELS={
    "cultivation":"Agriculture","environmental":"Environmental Systems","extraction":"Extraction",
    "fabrication":"Fabrication","infrastructure":"Infrastructure","research":"Research",
...
ITEM_EFFECTS={
    "toolkit":"+2 percentage points success for Fabrication/Infrastructure; keep one",
    "sensor":"+2 percentage points success for Extraction/Research/Frontier; keep one",
    "crate":"+1 SC on successful Logistics/Commerce actions; keep one",
    "ration":"Eat for +3 percentage points success for 10 minutes; time stacks",
    "water_filter":"+2 percentage points Environmental success; keep one",
    "siro_sampler":"+2 percentage points Research success; keep one",
    "duck_crate":"+2 percentage points Logistics success; keep one",
    "spaceport_manifest":"+2 percentage points Commerce success; keep one",
}
...
QUALITY_RECIPES={
    ...
    "cultivation_rig":{"name":"Agriculture Rig","category":"agriculture","cost":{"components":4,"rare_ore":1},"skills":{"cultivation":2},"special":"Agriculture XP"},
    ...
    "assembly_bench":{"name":"Portable Assembly Bench","category":"fabrication","cost":{"components":4,"rare_ore":1},"skills":{"fabrication":2,"infrastructure":1},"special":"Fabrication XP"},
...
WORLD_CONDITIONS=[
    ...
    ("water_watch","Water Watch","Environmental work matters more than usual."),
...
HOBBIES={
    ...
    "cooking":("Cooking","cultivation"),"collecting":("Collecting","frontier"),
...
STORY_ARCS=[
    {"key":"whispering_rows","name":"The Whispering Rows","text":"Unusual growth is spreading through greenhouse rows. Is it adaptation, contamination, or something stranger?","tracks":[("Agriculture Response",{"cultivation","environmental"}),("Research Response",{"research"}),("Supply Response",{"logistics","commerce"})],"goal":240},
...
def story_contribute(db,p,skill):
    ...
    if total>=cfg["goal"]:
        ...
        soc=society(db,p.channel_id);rewards=[("Agriculture/Environmental",("food",30)),("Research",("knowledge",30)),("Logistics/Commerce",("treasury",30))]
...
def skill_xp(p,skill):
    return {
        "cultivation":p.farm_xp,"environmental":p.environmental_xp,
        "extraction":p.mining_xp,"fabrication":p.fabrication_xp,
        "infrastructure":p.infrastructure_xp,"research":p.research_xp,
        "logistics":p.delivery_xp,"frontier":p.explore_xp,
        "commerce":p.commerce_xp,
    }[skill]
...
def task_need_gate(db,p,action,provider="twitch",life=None):
    ...
    return f"⛔ TASK BLOCKED: {shown} did not start. {details}. Nothing was consumed, no rewards rolled, and no cooldown started."
...
def near_milestone_note(db,p,skill):
    if not skill:return ""
    xp=skill_xp(p,skill);threshold=next((x for x in (10,25,50,90) if x>xp),None)
...
def shift_modifier(pw,day,skill):
    ...
    mapping={"field_duty":"cultivation","spaceport_duty":"logistics","lab_duty":"research",
...
def guide_action(db,p,skill,provider):
    if skill=="fabrication":return (("/make" if provider=="discord" else "!make"),0)
...
def daily(db,p):
    ...
    if not d:
        a=random.choice(["harvest","mine","research","make","delivery","explore","water","repair"]);t=random.choice([3,4,5])
...
def merge_accounts(db,channel,source_uid,target_uid):
    ...
    if source and not target:
        ...
        additive=["sc","contribution","farm_xp","mining_xp","industry_xp","research_xp","delivery_xp","explore_xp","environmental_xp","fabrication_xp","infrastructure_xp","commerce_xp","crops","ore","rare_ore","components","cargo","actions","successes"]
...
def profile(channel:str,uid:str,name:str="Citizen",provider:str="twitch"):
    with SessionLocal() as db:
        ...
        discord=(f"🌱 Seedling {p.display_name} — {identity}\n\n"
                 ...
                 f"🌿 Life: Agriculture L{lvl(p.farm_xp)} · Environmental L{lvl(p.environmental_xp)}\n"
                 f"🏭 Production: Extraction L{lvl(p.mining_xp)} · Fabrication L{lvl(p.fabrication_xp)} · Infrastructure L{lvl(p.infrastructure_xp)}\n"
...
def skills(channel:str,uid:str,name:str="Citizen",provider:str="twitch"):
    with SessionLocal() as db:
        _,p=player(db,channel,provider,uid,name)
        rows=[("cultivation","🌱","Agriculture",p.farm_xp),("environmental","💧","Environmental Systems",p.environmental_xp),("extraction","⛏️","Extraction",p.mining_xp),("fabrication","⚙️","Fabrication",p.fabrication_xp),("infrastructure","🏗️","Infrastructure",p.infrastructure_xp),("research","🔬","Research",p.research_xp),("logistics","📦","Logistics",p.delivery_xp),("frontier","🧭","Frontier Operations",p.explore_xp),("commerce","🏪","Commerce",p.commerce_xp)]
        specs={r.skill:SPECIALIZATIONS.get(r.skill,{}).get(r.choice,r.choice.replace("_"," ").title()) for r in db.execute(select(Specialization).where(Specialization.channel_id==channel,Specialization.canonical_uid==p.twitch_uid)).scalars().all()}
        strongest=max(rows,key=lambda r:r[3]);lowest=min(rows,key=lambda r:r[3])
        if provider=="discord":
            line=lambda key,e,label,xp:f"{e} {label} — Lv. {lvl(xp)} · {xp} XP"+(f" · {specs[key]}" if key in specs else "")
            return PlainTextResponse(f"🧬 {p.display_name} — Competencies (Aptitudes)\n\nPersonal strength: {strongest[2]} Lv. {lvl(strongest[3])}\nBest training opportunity: {lowest[2]} Lv. {lvl(lowest[3])}\n\n🌿 LIFE SYSTEMS\n"+"\n".join(line(*r) for r in rows[:2])+"\n\n🏭 PRODUCTION\n"+"\n".join(line(*r) for r in rows[2:5])+"\n\n🛰️ OPERATIONS & ECONOMY\n"+"\n".join(line(*r) for r in rows[5:])+"\n\nUse /guide goal:aptitude for your recommended training action.")
        return out(f"🧬 {p.display_name} Aptitudes | "+" · ".join(f"{e}{label} L{lvl(xp)} ({xp})" for _,e,label,xp in rows))
...
def guide_three_steps(db,p,s,w,clock,provider):
    ...
    if life.energy<TASK_NEED_MINIMUM:steps.append(f"Use {prefix}sleep to raise Energy above {TASK_NEED_MINIMUM}.")
    if life.nutrition<TASK_NEED_MINIMUM:steps.append(f"Use {prefix}eat; emergency food is available if you own none.")
    if life.social<TASK_NEED_MINIMUM:steps.append(f"Use {prefix}games to raise Social above {TASK_NEED_MINIMUM}.")
...
def job(channel:str,uid:str,name:str="Citizen",job:str="",provider:str="twitch"):
    with SessionLocal() as db:
        ...
        if job not in JOBS:return out("💼 Jobs: farmer, miner, technician, researcher, courier, explorer, merchant.")
...
def craft_missing_materials(db,p,costs):
    ...
    for key,amount in costs.items():
        ...
        if have<amount:missing.append(f"{amount-have} {resource_name(key)}")
    return missing

def craft_reward(db,p,s,kind):
    cfg=CRAFT_PAY[kind];job_bonus=1 if p.job=="technician" else 0
    ...
    return {"sc":cfg["sc"]+job_bonus,"xp":xp,"contribution":cfg["contribution"],"development":cfg["development"],"job_bonus":job_bonus}
...
def craft_menu(db,p,channel,provider,category=""):
    ...
    if provider!="discord":
        ...
        ready=[key for key,_,cost,_,need in rows if (need is None or tier_index>=need) and not unique_bonus_owned(db,p,key) and not craft_missing_materials(db,p,cost)]
...
def make(channel:str,uid:str,name:str="Citizen",recipe:str="",provider:str="twitch",category:str=""):
    ...
    with SessionLocal() as db:
        c,p=player(db,channel,provider,uid,name)
        if not recipe:return craft_menu(db,p,channel,provider,category)
        ...
        if recipe not in PART_RECIPES and recipe not in RECIPES and recipe not in QUALITY_RECIPES:
            return out("⚙️ Unknown recipe. Open /make with no options, then choose a crafting category.")
        ...
        if recipe in QUALITY_RECIPES:
            ...
            rewards=craft_reward(db,p,society(db,channel),"quality")
            ...
            text=(f"🛠️ FABRICATION COMPLETE — {p.display_name}\n\n"
                  f"OUTPUT\n• {quality} {r['name']} · Condition 100%\n• {detail}\n\n"
                  f"CHANGE\n• {cost_text(costs)[1:-1]} · -2 Energy · -1 Nutrition · -1 Comfort\n"
                  f"• +{rewards['xp']} Fabrication XP · +{rewards['sc']} SC · +{rewards['contribution']} Contribution"
                  +(f" · Technician bonus included" if rewards['job_bonus'] else "")+
        ...
        kind="components" if recipe in PART_RECIPES else "core";rewards=craft_reward(db,p,society_state,kind);craft_record(db,p,recipe)
        ...
        text=(f"⚙️ FABRICATION COMPLETE — {p.display_name}\n\n"
              ...
              f"CHANGE\n• {cost_text(costs)[1:-1]} · -2 Energy · -1 Nutrition · -1 Comfort\n"
              f"• +{rewards['xp']} Fabrication XP · +{rewards['sc']} SC"
...
@colony_command
def seed_industries(channel:str,uid:str,name:str="Citizen",action:str="browse",item_name:str="",amount:int=1,provider:str="twitch"):
    action=(action or "browse").lower().strip();key=(item_name or "").lower().strip().replace(" ","_");amount=max(1,min(25,int(amount or 1)))
    if action not in {"browse","buy","sell","orders","fulfill"}:return out("🏭 Seed Industries actions: browse, buy, sell, orders, fulfill.")
    if action=="browse":
        rows=[f"• {resource_name(k)} — Buy {v['buy']} SC · Sell {v['sell']} SC · {v['purpose']}" for k,v in SEED_INDUSTRIES.items()]
        discord=("🏭 SEED INDUSTRIES — MATERIAL EXCHANGE\n\n"
                 "Fixed-price supplies for citizens who need one missing link in a production chain. "
                 "Buying costs more than producing it yourself.\n\n"+"\n".join(rows)+
                 "\n\nUse /seedindustries action:Buy or action:Sell. Maximum 25 per transaction.")
        twitch="🏭 Seed Industries | "+" | ".join(f"{resource_name(k)} B{v['buy']}/S{v['sell']}" for k,v in SEED_INDUSTRIES.items())
        return platform_response(provider,discord,twitch)
...
def duo(channel:str,uid:str,name:str="Citizen",target:str="",activity:str="walk",provider:str="twitch"):
    ...
    with SessionLocal() as db:
        ...
        if familiarity<need:return out(f"🤝 {act.title()} requires relationship {need}. You currently have {familiarity}.")
...
def shift(channel:str,uid:str,name:str="Citizen",role:str="",provider:str="twitch"):
    ...
    with SessionLocal() as db:
        ...
        bonus_note="No shift bonus while off duty." if key=="off_duty" else "Matching work gets +2 percentage points success today."
...
def mentor(channel:str,uid:str,name:str="Citizen",target:str="",provider:str="twitch"):
    with SessionLocal() as db:
        ...
        fields=[("cultivation",target_p.farm_xp),("extraction",target_p.mining_xp),("fabrication",target_p.fabrication_xp),("research",target_p.research_xp),("logistics",target_p.delivery_xp),("frontier",target_p.explore_xp),("commerce",target_p.commerce_xp)]
...
def action(action:str,channel:str,uid:str,name:str="Citizen",msg:str="",provider:str="twitch"):
    with SessionLocal() as db:
        ...
        def fail(txt):
            ...
            message=txt+" No rewards."+grit+injury+exposure+encounter+modifier_note+life_change_summary(life_before,life,provider)+task_readiness_warning(life,provider)+improvement+(" "+auto if auto else "")
        ...
        def passed(base):
            ...
            chance_used[0]=min(.92,max(.10,success_chance(db,p,skill,base)+(0.05 if relevant else 0)+life_bonus+determination_used[0]))
            roll=random.random()
            p._practice_quality=1.2 if roll<chance_used[0]*.25 else 1.0
            return roll<chance_used[0]
        if action=="businessinvest":
            ...
            base=f"🌾 {p.display_name} completes {action_display_name(action,mode)}. +{xp_gain} Agriculture XP | +{2+bonus} SC | +{contribution_gain} Contribution | New Eridian gains +1 Food."
            ...
            base=f"💧 {p.display_name} completes {action_display_name(action,mode)}. +{xp_gain} Environmental Systems XP | +{2+bonus} SC | +{contribution_gain} Contribution | New Eridian gains {society_gain}."
            ...
            base=f"⛏️ {p.display_name} completes {action_display_name(action,mode)}. +{xp_gain} Extraction XP | +{sc_gain} SC | +{contribution_gain} Contribution | New Eridian gains +{2 if action=='rare' else 1} Materials."
            ...
            base=f"⚙️ {p.display_name} completes {action_display_name(action,mode)}. +{xp_gain} Fabrication XP | +{3+bonus} SC | +{contribution_gain} Contribution | New Eridian gains {society_gain}."
            ...
            base=f"🏗️ {p.display_name} completes {action_display_name(action,mode)}. +{xp_gain} Infrastructure XP | +{3+bonus} SC | +{contribution_gain} Contribution | New Eridian gains +1 Development."
...
Standard work, /eat, and /sleep use a 5-second cooldown. Social and recovery actions use 20–60 seconds depending on the activity. Browsing or crafting with /make does not use a cooldown. Check exact remaining times with /me section:Cooldowns.""",
...
/seedindustries — Fixed-price NPC exchange plus three rotating daily Production Orders. Orders consume manufactured goods and reward SC, Contribution, Development, Fabrication XP, and Commerce XP.
...
/agriculture — General Agriculture work: +1 XP, 2 SC before bonuses, +1 Contribution, and +1 society Food on success.
/agriculture action:Harvest Crops — Agriculture work that gives a personal Crop.
/agriculture action:Irrigate — Environmental work that adds society Food.
/agriculture action:Hydroponics — Requires a Water Filter and improves Food while producing a Crop.
/scan — Environmental work that adds +1 society Knowledge.
/mine — Reliable Extraction work. Success gives 1 Ore, Materials, XP, SC, and Contribution.
/rare — Risky 28% base search. Success gives 1 Rare Ore, +2 Materials, and 6 SC before bonuses.
/make — The complete Fabrication system. Components, finished supplies, quality gear, Fabrication XP, SC, and Development all live here.
/repair target:Society Infrastructure — Infrastructure work; adds +1 Development.
...
Old work routes remain compatible on Twitch/API, including !craft and !fabricate, but Discord uses /make as the single Fabrication system.""",
...
SC / Settlement Currency (legacy Seed Coin) — Personal currency used for businesses and Habitat upgrades.
Contribution — Personal society-service score used by the leaderboard.
XP — Stored competency practice. Successful tasks improve relevant competencies; occupation, living conditions, project work and outcome quality affect gain. Fractional practice carries forward. Lv. 10 unlocks specialization.
Competency / Aptitude — A skill family such as Agriculture, Research, or Logistics. Its level improves success chance.
...
def discord_seed_help(topic="overview",name="Citizen"):
    ...
    if topic in SEED_HELP_TOPICS:return greeting+SEED_HELP_TOPICS[topic]
...
def twitch_seed(topic:str="overview",page:str="1"):
    ...
    content=re.sub(r"/([a-z]+)",r"!\1",SEED_HELP_TOPICS[topic])
    if topic=="character":
        content+=" Twitch linking: !link creates a code; claim it with /link in Discord."
...
def _discord_action_embed(content,command,status):
    ...
    for i,piece in enumerate(pieces):
        ...
        if piece.startswith("Competency practice:"):progress.append(piece);continue
...
DISCORD_OPTION_SCHEMA = {'seed': [{'type': 3, 'name': 'topic', 'description': 'What do you want explained?', 'required': False, 'choices': [{'name': 'Start Here', 'value': 'start'}, {'name': 'Character & Progression', 'value': 'character'}, {'name': 'Home, Business & Crafting', 'value': 'property'}, {'name': 'Life & Social', 'value': 'life'}, {'name': 'Work Actions', 'value': 'production'}, {'name': 'Logistics, Frontier & Commerce', 'value': 'operations'}, {'name': 'Society & World', 'value': 'society'}, {'name': 'Other Player Actions', 'value': 'other'}, {'name': 'Moderator Controls', 'value': 'moderator'}, {'name': 'Terms & Definitions', 'value': 'terms'}]}], 'guide': [{'type': 3, 'name': 'goal', 'description': 'What are you trying to accomplish?', 'required': False, 'choices': [{'name': 'Best thing to do now', 'value': 'auto'}, {'name': 'Help the active event', 'value': 'event'}, {'name': 'Daily Contract', 'value': 'daily'}, {'name': 'Earn Seed Coin', 'value': 'seed_coin'}, {'name': 'Advance New Eridian', 'value': 'society'}, {'name': 'Train an aptitude', 'value': 'aptitude'}, {'name': 'Gather and craft', 'value': 'crafting'}, {'name': 'Habitat', 'value': 'home'}, {'name': 'Business', 'value': 'business'}, {'name': 'Fix life needs', 'value': 'life'}, {'name': 'Weekly Story', 'value': 'story'}, {'name': 'Market', 'value': 'market'}]}], 'start': [], 'me': [{'type': 3, 'name': 'section', 'description': 'Which part of your citizen?', 'required': False, 'choices': [{'name': 'Overview', 'value': 'overview'}, {'name': 'Life Needs', 'value': 'life'}, {'name': 'Bonuses', 'value': 'bonuses'}, {'name': 'Cooldowns', 'value': 'cooldowns'}, {'name': 'Traits', 'value': 'traits'}, {'name': 'Relationships', 'value': 'relationships'}, {'name': 'Journal', 'value': 'journal'}, {'name': 'First Days Tutorial', 'value': 'tutorial'}, {'name': 'Titles', 'value': 'titles'}, {'name': 'Display Style & Color Key', 'value': 'display'}]}, {'type': 3, 'name': 'title', 'description': 'Optional unlocked title to equip when section is Titles', 'required': False, 'autocomplete': True}, {'type': 3, 'name': 'style', 'description': 'Action result detail when section is Display Style', 'required': False, 'choices': [{'name': 'Compact', 'value': 'compact'}, {'name': 'Detailed', 'value': 'detailed'}]}], 'progress': [{'type': 3, 'name': 'section', 'description': 'Which progression system?', 'required': False, 'choices': [{'name': 'Skills & Level Unlocks', 'value': 'skills'}, {'name': 'Daily Contract', 'value': 'daily'}, {'name': 'Achievements', 'value': 'achievements'}, {'name': 'Collection', 'value': 'collection'}]}], 'inventory': [{'type': 3, 'name': 'section', 'description': 'What do you want to inspect?', 'required': False, 'choices': [{'name': 'Everything', 'value': 'all'}, {'name': 'Quality Gear', 'value': 'gear'}]}], 'job': [{'type': 3, 'name': 'job', 'description': 'Profession', 'required': True, 'choices': [{'name': 'Farmer', 'value': 'farmer'}, {'name': 'Miner', 'value': 'miner'}, {'name': 'Technician', 'value': 'technician'}, {'name': 'Researcher', 'value': 'researcher'}, {'name': 'Courier', 'value': 'courier'}, {'name': 'Explorer', 'value': 'explorer'}, {'name': 'Merchant', 'value': 'merchant'}]}], 'specialize': [{'type': 3, 'name': 'path', 'description': 'Specialization path', 'required': True, 'choices': [{'name': 'Agriculture — Crop Geneticist', 'value': 'cultivation:crop_geneticist'}, {'name': 'Agriculture — Hydroponics Operator', 'value': 'cultivation:hydroponics_operator'}, {'name': 'Environmental — Water Systems Specialist', 'value': 'environmental:water_specialist'}, {'name': 'Environmental — Biosphere Warden', 'value': 'environmental:biosphere_warden'}, {'name': 'Extraction — Deep-Core Prospector', 'value': 'extraction:deep_core_prospector'}, {'name': 'Extraction — Rare Materials Surveyor', 'value': 'extraction:rare_materials_surveyor'}, {'name': 'Fabrication — Precision Fabricator', 'value': 'fabrication:precision_fabricator'}, {'name': 'Fabrication — Machine Integrator', 'value': 'fabrication:machine_integrator'}, {'name': 'Infrastructure — Habitat Engineer', 'value': 'infrastructure:habitat_engineer'}, {'name': 'Infrastructure — Utility Architect', 'value': 'infrastructure:utility_architect'}, {'name': 'Research — Siro Analyst', 'value': 'research:siro_analyst'}, {'name': 'Research — Materials Researcher', 'value': 'research:materials_researcher'}, {'name': 'Logistics — Spaceport Coordinator', 'value': 'logistics:spaceport_coordinator'}, {'name': 'Logistics — Delivery Fleet Operator', 'value': 'logistics:delivery_fleet_operator'}, {'name': 'Frontier — Avesta Pathfinder', 'value': 'frontier:pathfinder'}, {'name': 'Frontier — Field Surveyor', 'value': 'frontier:field_surveyor'}, {'name': 'Commerce — Market Broker', 'value': 'commerce:market_broker'}, {'name': 'Commerce — Business Steward', 'value': 'commerce:business_steward'}]}], 'home': [{'type': 3, 'name': 'action', 'description': 'Habitat action', 'required': False, 'choices': [{'name': 'View Habitat', 'value': 'view'}, {'name': 'Upgrade Habitat', 'value': 'upgrade'}]}], 'business': [{'type': 3, 'name': 'action', 'description': 'Business action', 'required': False, 'choices': [{'name': 'View Business', 'value': 'view'}, {'name': 'Start Business', 'value': 'start'}, {'name': 'Work', 'value': 'work'}, {'name': 'Contract', 'value': 'contract'}, {'name': 'Invest', 'value': 'invest'}]}, {'type': 3, 'name': 'name', 'description': 'Business name (only used when starting)', 'required': False, 'min_length': 1, 'max_length': 30}], 'make': [{'type': 3, 'name': 'category', 'description': 'Production stage or complete item tree', 'required': False, 'choices': [{'name': 'Basic Components', 'value': 'basic_components'}, {'name': 'Advanced Components', 'value': 'advanced_components'}, {'name': 'Final Products', 'value': 'final_products'}, {'name': 'Raw Materials', 'value': 'raw_materials'}, {'name': 'Production Tree', 'value': 'tree'}]}, {'type': 3, 'name': 'recipe', 'description': 'Optional: select to craft; with Production Tree, preview only', 'required': False, 'autocomplete': True}], 'seedindustries': [{'type': 3, 'name': 'action', 'description': 'Trade materials or manage production orders', 'required': False, 'choices': [{'name': 'Browse Market', 'value': 'browse'}, {'name': 'Buy', 'value': 'buy'}, {'name': 'Sell', 'value': 'sell'}, {'name': 'View Production Orders', 'value': 'orders'}, {'name': 'Fulfill Production Order', 'value': 'fulfill'}]}, {'type': 3, 'name': 'item', 'description': "Choose action first: trade material or today's production order", 'required': False, 'autocomplete': True}, {'type': 4, 'name': 'amount', 'description': 'Quantity (1-25)', 'required': False, 'min_value': 1, 'max_value': 25}], 'world': [{'type': 3, 'name': 'section', 'description': 'World information', 'required': False, 'choices': [{'name': 'Overview', 'value': 'overview'}, {'name': 'Conditions & Siro', 'value': 'conditions'}, {'name': 'Daily Bulletin', 'value': 'bulletin'}, {'name': 'Rumor', 'value': 'rumor'}, {'name': 'Weekly Story', 'value': 'story'}, {'name': 'Society Project', 'value': 'project'}, {'name': 'Market', 'value': 'market'}]}], 'society': [{'type': 3, 'name': 'section', 'description': 'Society information', 'required': False, 'choices': [{'name': 'Overview', 'value': 'overview'}, {'name': 'Next Tier Progress', 'value': 'progress'}, {'name': 'Contribution Leaderboard', 'value': 'leaderboard'}]}], 'event': [{'type': 3, 'name': 'section', 'description': 'Event information', 'required': False, 'choices': [{'name': 'Active Event', 'value': 'status'}, {'name': 'Recent Event History', 'value': 'history'}]}], 'district': [{'type': 3, 'name': 'district', 'description': 'Home district', 'required': True, 'choices': [{'name': 'Residential Ring', 'value': 'residential_ring'}, {'name': 'Agricultural District', 'value': 'agricultural_district'}, {'name': 'Industrial Ward', 'value': 'industrial_ward'}, {'name': 'Research Block', 'value': 'research_block'}, {'name': 'Market Concourse', 'value': 'market_concourse'}, {'name': 'Spaceport Quarter', 'value': 'spaceport_quarter'}, {'name': 'Frontier Edge', 'value': 'frontier_edge'}]}], 'shift': [{'type': 3, 'name': 'role', 'description': 'Shift role', 'required': True, 'choices': [{'name': 'Off Duty', 'value': 'off_duty'}, {'name': 'Field Duty', 'value': 'field_duty'}, {'name': 'Spaceport Duty', 'value': 'spaceport_duty'}, {'name': 'Lab Duty', 'value': 'lab_duty'}, {'name': 'Maintenance', 'value': 'maintenance'}, {'name': 'Trade Duty', 'value': 'trade_duty'}, {'name': 'Survey Duty', 'value': 'survey_duty'}]}], 'social': [{'type': 3, 'name': 'action', 'description': 'Social activity', 'required': False, 'choices': [{'name': 'Say Hi', 'value': 'hi'}, {'name': 'Hang Out', 'value': 'hangout'}, {'name': 'Mentor', 'value': 'mentor'}, {'name': 'Duo Walk', 'value': 'duo_walk'}, {'name': 'Duo Games', 'value': 'duo_games'}, {'name': 'Duo Research', 'value': 'duo_research'}, {'name': 'Duo Delivery', 'value': 'duo_delivery'}, {'name': 'Duo Explore', 'value': 'duo_explore'}, {'name': 'Use Recreation Set', 'value': 'group_games'}]}, {'type': 3, 'name': 'player', 'description': 'Citizen required for relationship actions', 'required': False, 'autocomplete': True}], 'relax': [], 'walk': [], 'games': [], 'hobby': [{'type': 3, 'name': 'hobby', 'description': 'Hobby', 'required': True, 'choices': [{'name': 'Gardening', 'value': 'gardening'}, {'name': 'Exploration', 'value': 'exploration'}, {'name': 'Mechanics', 'value': 'mechanics'}, {'name': 'Research', 'value': 'research'}, {'name': 'Games', 'value': 'games'}, {'name': 'Rockwatching', 'value': 'rockwatching'}, {'name': 'Cooking', 'value': 'cooking'}, {'name': 'Collecting', 'value': 'collecting'}, {'name': 'Trading', 'value': 'trading'}, {'name': 'Scanning', 'value': 'scanning'}]}], 'meal': [], 'eat': [], 'sleep': [], 'use': [{'type': 3, 'name': 'item', 'description': 'Select an owned consumable; craft more with /make', 'required': True, 'autocomplete': True}], 'ducks': [{'type': 3, 'name': 'duck', 'description': 'Optional preferred delivery partner', 'required': False, 'choices': [{'name': 'Hueburt', 'value': 'Hueburt'}, {'name': 'Colora', 'value': 'Colora'}, {'name': 'Prisma', 'value': 'Prisma'}, {'name': 'Pastelle', 'value': 'Pastelle'}, {'name': 'Asimov', 'value': 'Asimov'}]}], 'link': [{'type': 3, 'name': 'code', 'description': 'Six-character code from Twitch !link', 'required': True}], 'linklookup': [{'type': 3, 'name': 'player', 'description': 'Select a player or enter a provider ID', 'required': False, 'autocomplete': True}], 'eventstart': [{'type': 3, 'name': 'event', 'description': 'Event', 'required': True, 'choices': [{'name': 'Siro Bloom', 'value': 'siro'}, {'name': 'Food Crisis', 'value': 'food'}, {'name': 'Mining Boom', 'value': 'mining'}, {'name': 'Delivery Surge', 'value': 'delivery'}, {'name': 'Market Boom', 'value': 'market'}, {'name': 'Water Treatment Emergency', 'value': 'water'}, {'name': 'Infrastructure Breakdown', 'value': 'machine'}, {'name': 'Spaceport Rush', 'value': 'spaceport'}]}], 'eventstop': [], 'modlog': [], 'agriculture': [{'type': 3, 'name': 'action', 'description': 'Agriculture activity', 'required': False, 'choices': [{'name': 'Tend Fields', 'value': 'tend'}, {'name': 'Harvest Crops', 'value': 'harvest'}, {'name': 'Irrigate', 'value': 'irrigate'}, {'name': 'Hydroponics (Water Filter)', 'value': 'hydroponics'}]}], 'scan': [], 'mine': [], 'rare': [], 'research': [{'type': 3, 'name': 'operation', 'description': 'Research operation', 'required': False, 'choices': [{'name': 'Standard Research', 'value': 'standard'}, {'name': 'Field Analysis (Siro Sampler)', 'value': 'field_analysis'}]}], 'repair': [{'type': 3, 'name': 'target', 'description': 'What needs repair?', 'required': False, 'choices': [{'name': 'Society Infrastructure', 'value': 'society'}, {'name': 'Personal Quality Gear', 'value': 'gear'}]}, {'type': 3, 'name': 'item', 'description': 'Owned gear key when target is Personal Quality Gear', 'required': False, 'autocomplete': True}], 'cargo': [], 'delivery': [], 'spaceport': [{'type': 3, 'name': 'operation', 'description': 'Spaceport operation', 'required': False, 'choices': [{'name': 'Standard Operations', 'value': 'standard'}, {'name': 'Expedite (-1 Power Cell)', 'value': 'expedite'}]}], 'explore': [{'type': 3, 'name': 'operation', 'description': 'Frontier operation', 'required': False, 'choices': [{'name': 'Scout', 'value': 'scout'}, {'name': 'Advanced Survey (Sensor)', 'value': 'survey'}]}], 'market': [{'type': 3, 'name': 'action', 'description': 'Market action', 'required': False, 'choices': [{'name': 'View Prices', 'value': 'view'}, {'name': 'Sell Resources', 'value': 'sell'}, {'name': 'Commerce Work', 'value': 'work'}, {'name': 'Analyze Market (Market Analyzer)', 'value': 'analyze'}]}, {'type': 3, 'name': 'resource', 'description': 'Resource to sell', 'required': False, 'choices': [{'name': 'Crops', 'value': 'crops'}, {'name': 'Ore', 'value': 'ore'}, {'name': 'Rare Ore', 'value': 'rare_ore'}, {'name': 'Components', 'value': 'components'}, {'name': 'Cargo', 'value': 'cargo'}]}, {'type': 4, 'name': 'amount', 'description': 'Amount to sell (1-25)', 'required': False, 'min_value': 1, 'max_value': 25}]}
    ...
    if command=="seedindustries" and option=="item":
        ...
        with SessionLocal() as db:
            if mode=="fulfill":
                ...
                rows=[(f"{resource_name(key)} — {data[mode]} SC each",key) for key,data in SEED_INDUSTRIES.items()]
            else:rows=[]
...
def _discord_validate_options(command,options):
    ...
    for key,value in options.items():
        ...
        if field.get('type')==4:
            if isinstance(value,bool) or not isinstance(value,int) or not field.get('min_value',1)<=value<=field.get('max_value',25):
                return options,f"⚠️ {key.title()} must be a whole number from 1 to 25. Nothing spent."
    ...
    restrictions={
        ...
        'seedindustries':('action',{'item':{'buy','sell','fulfill'},'amount':{'buy','sell'}}),
