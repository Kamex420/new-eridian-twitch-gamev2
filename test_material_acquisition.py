...
def test_each_raw_material_is_gatherable_by_a_new_citizen(key):
    with m.SessionLocal() as db:
        ...
        assert s.source_hint(key).startswith('/gather resource:')


def test_gather_pages_cover_every_resource_and_search_finds_26th():
...
def test_discord_gather_pages_and_twitch_template_contracts():
    ...
    rows={r['command']:r['response'] for r in csv.DictReader(Path('streamelements_commands.csv').open(encoding='utf-8-sig'))}
